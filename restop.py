			...
			for neighbor in a2.GetNeighbors():
				if neighbor.GetIdx()!=a1.GetIdx():
					...
					if angle not in seen and tuple(reversed(angle)) not in seen:
						...
						angles.append(angle)
		return angles  
>>>>>>> efcc47c6d6fe2fc7c3ac77a1da6df15e8a3ba19d
	...
	def CheckBonds(self):
		...
		for bond in mol.GetBonds():
			...
			if not self.ff.Bonded.bonds[atypes] and not self.ff.Bonded.bonds[reversed(atypes)]:
		...
		if self.extraff:
			with open(fname, 'w') as f:
				...
				for angle in  self.extraff.Bonded.angles:
					atype = self.extraff.Bonded.angles[angle]
					f.write('%8s %8s %8s' %angle + ' %5s %12s %12s %12s %12s'%(atype.func, ('%.6e' % Decimal(atype.angle)), ('%.4e' % Decimal(atype.fconstant)), ('%.4e' % Decimal(atype.ubval)), ('%.4e' % Decimal(atype.ubfconstant)) )+'\n')
				f.write('\n [ dihedraltypes ] \n')
				f.write(';'+'%7s %8s %8s %8s %5s %12s %12s %5s \n' %('i','j','k','l','func','phi0','kphi','mult'))
				for dihedral in  self.extraff.Bonded.dihedrals:
					dtype = self.extraff.Bonded.dihedrals[dihedral]
					f.write('%8s %8s %8s %8s' %dihedral + '%5s %14s %14s %6s'%(dtype.func, ('%.6e' % Decimal(dtype.angle)), ('%.6e' % Decimal(dtype.fconstant)), dtype.mult) + '\n')
				f.write('\n [ dihedraltypes ] \n')
